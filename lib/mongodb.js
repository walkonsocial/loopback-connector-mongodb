...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  /*var dao = dataSource.constructor.DataAccessObject;

  dao.bulkUpsert = MongoDB.prototype.bulkUpsert.bind(dataSource.connector);

  /!*var m = 'bulkUpsert';
  if ('function' === typeof method) {
    DataAccessObject[m] =
      method.bind(dataSource.connector);
    for (var k in method) {
      DataAccessObject[m][k] = method[k];
    }
  }

  dataSource.connector.DataAccessObject = DataAccessObject;*!/*/
...
MongoDB.prototype.connect = function (callback) {
  ...
  if (self.db) {
    ...
    mongodb.MongoClient.connect(self.settings.url, self.settings,
      function (err, db) {
        if (!err) {
          if (self.debug) {
          ...
          if (self.debug || !callback) {
            console.error('MongoDB connection is failed: ' + self.settings.url,
              err);
          }
        }
...
MongoDB.prototype.parseUpdateData = function (model, data) {
  ...
  if (this.settings.allowExtendedOperators === true) {
    ...
    var acceptedOperators = [
      // Field operators
      '$currentDate', '$inc', '$max', '$min', '$mul', '$rename', '$setOnInsert',
      '$set', '$unset',
      // Array operators
 ...
 * @param {Function} [callback] The callback function
 */
MongoDB.prototype.updateOrCreate =
  function updateOrCreate(model, data, callback) {
    var self = this;
    if (self.debug) {
 ...
 * Update all matching instances
 * @param {String} model The model name
 * @param {{where:Object,update:Object}[]} updateList List of update items
 * @callback {Function} cb Callback function
 */
MongoDB.prototype.bulkUpsert = function bulkUpsert(model, updateList, cb) {
  var self = this;
  if (self.debug) {
    debug('bulkUpsert', model, data);
  }
  var idName = this.idName(model);

  var bulk = this.collection(model).initializeUnorderedBulkOp();

  for (var i in updateList) {
    var updateListItem = updateList[i];

    var where = self.buildWhere(model, updateListItem.where);
    // Check for other operators and sanitize the data obj
    var updateData = self.parseUpdateData(model, updateListItem.update);
    bulk.find(where).upsert().update(updateData);
  }
  bulk.execute(function (err, info) {
    if (err) return cb && cb(err);

    if (self.debug) {
      debug('bulkUpsert.callback', model, updateList, err, info);
    }
  ...
  if (self.db) {
    ...
    async.each(models, function (model, modelCallback) {
      ...
      async.each(indexList, function (index, indexCallback) {
        ...
        self.collection(model).ensureIndex(index.fields || index.keys,
          index.options, indexCallback);
      }, modelCallback);
